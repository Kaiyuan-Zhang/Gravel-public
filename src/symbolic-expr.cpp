...
namespace Symbolic {
    ...
    void ExprVisitor::visit(Expr &expr) {
        expr.accept(*this);
    }
    ...
    bool BitVecType::equal_to(std::shared_ptr<Type> t) const {
        if (t->is_bv_type()) { auto w = t->get_bv_width();
            return w == bitwidth;
        }
    ...
    Lambda::Lambda(std::shared_ptr<Type> func_type,
                   FuncT f) {
    ...
    Lambda::Lambda(const PtrList<Type> &arg_t,
...
#define BV_BIN_OP_CONCRETE(class_name, expr) \
    ...
    LNotExpr::LNotExpr(ExprPtr a): OpApplyNode({a}) {
        type = a->type;
    }

    NotExpr::NotExpr(ExprPtr a): OpApplyNode({a}) {
        type = a->type;
    }

    ExtractExpr::ExtractExpr(ExprPtr e, int start, int end): v(e), from(start), to(end) {
        ...
        type = std::dynamic_pointer_cast<Type>(std::make_shared<BitVecType>(to - from));
    }
    ...
    IteExpr::IteExpr(ExprPtr c, ExprPtr t, ExprPtr f): cond(c), t_val(t), f_val(f) {
    ...
    ForallExpr::ForallExpr(const std::vector<ExprPtr> &vs, ExprPtr c) {
        vars = vs;
        cond = c;
        type = std::make_shared<BitVecType>(1);
    }
