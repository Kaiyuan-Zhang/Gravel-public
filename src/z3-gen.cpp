...
namespace Symbolic {
    ...
    z3::expr Z3Expr::get_expr() {
        return std::get<z3::expr>(content);
    }
    ...
    z3::func_decl Z3Expr::get_func() {
        return std::get<z3::func_decl>(content);
    }
...
#define LOAD_BIN_OPRAND                     \
    ...
    VISITOR_IMPL(Expr) {}
    VISITOR_IMPL(Lambda) {
        throw "could not gen z3 expr from lambda";
    }
...
#undef VISITOR_IMPL
    ...
    bool verify_with_z3(Z3Context &ctx, ExprPtr pre_cond, ExprPtr target, bool do_fork) {
    ...
    void print_z3_expr(ExprPtr expr, std::ostream &os) {
        Z3Context ctx;
        os << gen_z3_expr(ctx, expr).get_expr().simplify();
    }
}
